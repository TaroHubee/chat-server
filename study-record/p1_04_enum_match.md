...
Rust の `enum` を使うとこれをエレガントに表現できます。
...
TypeScript の `switch` や Java の `if-else` では書き忘れをコンパイラが教えてくれません。Rust の `match` は**処理漏れをゼロにする**強力な仕組みです。
