...
チャットサーバーには TCP を使います。TCP は「電話」のようなプロトコルで、接続を確立してから双方向に通信します（UDPは「手紙」に近く、届いたかの確認をしません）。

チャットアプリにはメッセージが**確実に・順番通りに届く**ことが必要なので TCP を選びます。
...
学習中は使って構いませんが、本番コードでは `match` や `?` 演算子で丁寧に処理します。
...
接続が来た瞬間に OS が新しい `TcpStream` を作り、`(stream, addr)` として返してきます。
